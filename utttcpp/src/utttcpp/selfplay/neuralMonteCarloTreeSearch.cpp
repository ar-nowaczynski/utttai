...
NeuralMonteCarloTreeSearch::NeuralMonteCarloTreeSearch(
...
NeuralMonteCarloTreeSearch::~NeuralMonteCarloTreeSearch() { delete tree; }

void NeuralMonteCarloTreeSearch::run() {
...
Action NeuralMonteCarloTreeSearch::selectAction(
    ...
    if (selectionMethod == "argmax") {
        ...
        const int randomThreshold = randInt(1, totalVisitCount);
...
std::string NeuralMonteCarloTreeSearch::toString() const {
    std::string nmctsString = "NeuralMonteCarloTreeSearch(\n";
    nmctsString += "  tree: ";
    std::string treeString = tree->toString();
    for (std::string::size_type i = 0, j = 0; j < treeString.length(); j++) {
        if (treeString[j] == '\n') {
            nmctsString += treeString.substr(i, 1 + j - i);
            nmctsString += "  ";
            i = j + 1;
    ...
    nmctsString += "\n";
    nmctsString += "  num_simulations: " + std::to_string(numSimulations) + "\n";
    nmctsString += "  exploration_strength: " + std::to_string(explorationStrength) + "\n";
    nmctsString += ")";
    return nmctsString;
}

void NeuralMonteCarloTreeSearch::simulate() {
    selectLeafNode();
...
void NeuralMonteCarloTreeSearch::selectLeafNode() {
    Node *node = tree->root;
...
double NeuralMonteCarloTreeSearch::Q(const Node *node) const {
    return -node->stateValueMean;
}

double NeuralMonteCarloTreeSearch::U(const Node *node, const int parentVisitCount) const {
...
void NeuralMonteCarloTreeSearch::evaluate(Node *node, const double softmaxTemperature) {
    if (node->isTerminal()) {
    ...
    torch::Tensor utttStateTensor = getUtttStateTensor(node->uttt);
    utttStateTensor = utttStateTensor.to(torch::kCUDA).to(torch::kFloat32);
    std::vector<torch::jit::IValue> inputs;
    inputs.push_back(utttStateTensor);
    auto predictions = policyValueNet.forward(inputs).toTuple();
    torch::Tensor policyLogitsTensor = predictions->elements()[0].toTensor().to(torch::kCPU);
    torch::Tensor stateValueTensor = predictions->elements()[2].toTensor().to(torch::kCPU);
    torch::Tensor policyLogits = torch::zeros({node->childNodesLength});
    for (int i = 0; i < node->childNodesLength; i++) {
        int actionIndex = node->childNodes[i]->action.index;
        policyLogits[i] = policyLogitsTensor.index({0, ROW_INDEX[actionIndex], COL_INDEX[actionIndex]});
    }
    ...
    double stateValue = stateValueTensor[0].item<double>();
    node->stateValue = stateValue;
}

void NeuralMonteCarloTreeSearch::backprop(const double stateValue) {
    int sign = 1;
...
torch::Tensor NeuralMonteCarloTreeSearch::getUtttStateTensor(const UltimateTicTacToe *uttt) {
    torch::Tensor utttStateTensor =
        torch::zeros({1, 4, 9, 9}, torch::TensorOptions().dtype(torch::kI8));
    ...
    for (int si = 0; si < 81; si++) {
        if (state[si] == X_STATE_VALUE) {
            utttStateTensor.index_put_({0, xi, ROW_INDEX[si], COL_INDEX[si]}, 1);
        } else if (state[si] == O_STATE_VALUE) {
            utttStateTensor.index_put_({0, oi, ROW_INDEX[si], COL_INDEX[si]}, 1);
        }
    }
    utttStateTensor.index_put_({0, 2}, fillValue);
    int numLegalIndexes = uttt->findLegalIndexes(fillLegalIndexes);
    for (int i = 0, li; i < numLegalIndexes; i++) {
        li = fillLegalIndexes[i];
        utttStateTensor.index_put_({0, 3, ROW_INDEX[li], COL_INDEX[li]}, 1);
    }
    return utttStateTensor;
}
